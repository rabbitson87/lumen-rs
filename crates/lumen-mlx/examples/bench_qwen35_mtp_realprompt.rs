...
fn main() -> Result<()> {
    ...
    if let Ok(calib_out) = std::env::var("MTP_CALIB_OUT") {
        ...
        while emitted < calib_gen {
            let out = qwen.qwen35_mtp_step(seq_c, last_c, k, 0.0, 1.0, 0)?;
    ...
    for _ in 0..warmup {
        let w = qwen.qwen35_mtp_step(seq_b, last_b, k, mtp_temp, mtp_topp, 0)?;
    ...
    while emitted < n_gen {
        let t0 = Instant::now();
        let out = qwen.qwen35_mtp_step(seq_b, last_b, k, mtp_temp, mtp_topp, 0)?;
