...
pub(crate) mod imp {
    ...
    use crate::gemma4_moe::imp::{
        GenerateConfig, NativeGemma4Model, NativeGemma4PromptCache,
    };
    ...
    impl Gemma4Backend {
            ...
            let (mut cache, hit_kind, lcp) = match self.prefix_caches.get(prefix_cache_key) {
                Some(entry) => {
                    ...
                    if lcp > 0 && lcp <= entry.prefix_tokens.len() {
                        ...
                        if lcp < cache.offset() {
                            cache.truncate_to(lcp).context(
                                "chat_with_prefix_cache: truncate cloned master to LCP",
                            )?;
                        }
                        (cache, "hit", lcp)
                    } else {
        ...
        pub fn chat_streaming(
            ...
            eprintln!(
                ...
                n_chunks, chunk_size, prompt.len()
            );
            ...
            for (i, chunk) in chunks.into_iter().enumerate() {
                ...
                chunk_logits
                    .eval()
                    .with_context(|| format!(
                        "chat_streaming: prefill chunk {}/{} eval",
                        i + 1, n_chunks
                    ))?;
                ...
                eprintln!(
                    ...
                    i + 1, n_chunks, t0.elapsed().as_secs_f64() * 1000.0,
                    active as f64 / 1e9, peak as f64 / 1e9, cache as f64 / 1e9
                );
                if i + 1 == n_chunks {
            ...
            if let Some(ref p) = capture_path {
                ...
                eprintln!(
                    "[metal-capture] started: path={p} capture_steps={capture_steps}"
                );
            }
            ...
            loop {
                ...
                if kcache_stats_enabled {
                    let (hits, misses) =
                        mlx_rs::metal::kernel_cache_stats().unwrap_or((0, 0));
                    let (commits, cbops) =
                        mlx_rs::metal::cmd_buffer_stats().unwrap_or((0, 0));
            ...
            if capture_path.is_some() && !capture_stopped {
                ...
                eprintln!(
                    "[metal-capture] stopped at end-of-loop ({count} steps recorded)"
                );
            }
            ...
            if kcache_stats_enabled && count > 0 {
                ...
                let (sched_nt, sched_cc, sched_wait, sched_max_act) =
                    mlx_rs::metal::scheduler_stats().unwrap_or((0, 0, 0, 0));
                let (eval_calls, eval_ns) =
                    mlx_rs::metal::eval_gpu_stats().unwrap_or((0, 0));
                ...
                let sched_wait_us_per_step =
                    sched_wait_us / count as f64;
                ...
                if let Ok((b2f, f2b, noop, other)) =
                    mlx_rs::metal::astype_pair_stats()
                {
                    eprintln!(
                ...
                if let Ok(dump) = mlx_rs::metal::prim_histogram_dynamic() {
                    ...
                    eprintln!(
                        "[gemma4-primhist-dyn] top-{max_show} per-step:"
                    );
                    for (name, cnt) in entries.iter().take(max_show) {
                        let per_step = *cnt as f64 / c;
                        eprintln!(
                            "    {name:<30} total={cnt:>7} per_step={per_step:>7.1}"
                        );
                    }
                }
                eprintln!(
            ...
            if count_ops_enabled && step_samples > 0 {
                ...
                if breakdown_enabled {
                    ...
                    for (loc, count) in breakdown.iter().take(30) {
                        ...
                        eprintln!("  {count:>7}  ({pct:>5.1}%)  per_step={per_step:>6.1}  {loc}");
                    }
                }
                if timing_enabled {
    ...
    mod tests {
        ...
        const LMSTUDIO_DIR: &str =
            "/path/to/models/gemma-4-26b-a4b-mlx-4bit";
        ...
        fn backend_chat_streaming_delivers_chunks() {
            ...
            eprintln!("[backend-stream] chunks={:?} final={:?}", chunks, resp.visible);
            assert!(
                !chunks.is_empty(),
