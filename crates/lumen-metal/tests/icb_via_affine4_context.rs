...
fn affine4_context_icb_record_replay_residual() {
    ...
    let icb = IndirectCommandBuffer::new(&raw_device, N_PER_CB, 8).expect("ICB alloc");
    ...
    eprintln!("Projected 27B Dense decode: 67 ms → {new_ms:.2} ms = {new_tps:.2} tok/s");

    if savings_us_per_op < 4.0 {
        panic!("lumen API ICB savings {savings_us_per_op:.2} µs/op < expected ~10 µs/op");
    }
}
